...
\end{framed}
\textbf{Score function trick}: $\grad_\varphi \E[\tau\sim\Pi_\varphi]{G_0} = \E[\tau\sim\Pi_\varphi]{G_0 \grad_\varphi \log \Pi_\varphi(\tau)}=\E[\tau\sim\Pi_\varphi]{G_0  \sum_{t=0}^{T-1}\grad_\varphi\log \pi_\varphi(a_t|x_t)}$.\\
Has \textbf{high variance} unlike the reparametrization trick.\\
\textbf{Baseline}: $\E[\tau\sim\Pi_\varphi]{G_0\grad_\varphi\log\Pi_\varphi(\tau)}=\E[\tau\sim\Pi_\varphi]{\sum_{t=0}^{T-1}(G_0-b(\tau_{0:t-1}))\grad_\varphi\log\pi_\varphi(a_t|x_t)}$\\
\begin{framed}
...
\end{framed}
\textbf{On-policy AC}: learn critic through SARSA and actor through policy gradient methods\\
\includegraphics[width=0.68\linewidth, trim={0 0 4cm 0}]{images/Online_actor_critic.png}
...
Objective (\textbf{deterministic policy gradients}/\textbf{hill-climbing}): $\varphi^\star=\argmax_\varphi J_\mu(\varphi)=\argmax_\varphi \E[x\sim\mu]{Q^\star(x,\pi_\varphi(x);\theta)}$\\
Learn critic through \textbf{bootstrapped} Q-learning \\
...
\end{framed}
For randomized policies like \textbf{SVG}, we get:
...
$= \sum_{t=0}^\infty\E[(x_t,a_t)\sim\Pi_\varphi]{r(x_t,a_t)+\lambda\H{\pi_\varphi(\cdot|x_t)}}$\\
...
\textbf{Soft-Actor-Critic} and \textbf{MAP optimization} are off-policy actor-critic methods with entropy regularization.\\
